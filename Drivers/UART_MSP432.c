...
/*****************************************************************************
 * Function: activarInterrupcionConCharsErroneos_UART
 * Preconditions: None.
 * Overview: Pone en alto la interrupción por carácteres erróneos.
 * Input:  Bool que indique si la interrupción se habilita (1) o no (0).
 * Output: None.
 *****************************************************************************/
void activarInterrupcionConCharsErroneos_UART(bool interruption)
{
...
/*****************************************************************************
 * Function: establecerBitsDeParo_UART
 * Preconditions: None.
 ...
 * Output: None.
 *****************************************************************************/
void establecerBitsDeParo_UART(bool stop_bits)
{
...
/*****************************************************************************
 * Function: establecerDireccionTransmision_UART
 * Preconditions: None.
 * Overview: Establece si se debe contemplar sobremuestreo.
 * Input:  Bool que indique si se debe imprimir primero LSB (0) o MSB (1).
 * Output: None.
 *****************************************************************************/
void establecerDireccionTransmision_UART(bool direction)
{
...
/*****************************************************************************
 * Function: establecerPinesTxRx_UART
 * Preconditions: None.
 ...
 * Output: None.
 *****************************************************************************/
void establecerPinesTxRx_UART(uint32_t selected_port, uint32_t selected_pins)
{
 ...
 * Output: None.
 *****************************************************************************/
void establecerBaudRate_UART(uint8_t standard)
{
...
void inicializar_UART(void)
{
        ...
        establecerPinesTxRx_UART(PORT_1, PINES_PORT_1_UART);         /* Selección de pines. */
        establecerFuenteDeReloj_UART(SM_CLK);                                 /* Selección de la fuente de reloj. (Bit SMCLK). */
        establecerDireccionTransmision_UART(LSB_FIRST);                      /* Selección de MSB o LSB en orden de transmisión (0 para LSB, 1 para MSB). */
        establecerBitsDeParo_UART(ONE_STOP_BIT);                         /* Número de bits de paro; UCSPB = 0(1 stop bit) OR 1(2 stop bits). */
        establecerParidad_UART(NO_PARITY);                               /* Sin paridad entre la comunicación. */

        /* BaudRate trabajará a 115200. */
        establecerBaudRate_UART((uint8_t) BR_115200);
        establecerNumeroBitsDatos_UART(EIGHT_BITS);             /* Seleccion de caracter 8 bits. */
        establecerModo_UART(ASYNCHRONOUS);                /* Módo Asíncrono habilitado.    */
        establecerSobremuestreo_UART(OVERSAMPLE);      /* Sobremuestreo (Oversampling). */

        activarInterrupcionConCharsErroneos_UART(NO_INTERRUPTION);        /* No hay interrupción por caracteres erroneos.  */
        activarInterrupcionConBreakChars_UART(NO_INTERRUPTION);        /* No hay interrupción por caracteres 'break'.   */

        iniciar_UART(MAIN_UART);         /* Habilita el módulo. */
}
 ...
 *
 *****************************************************************************/
void iniciar_UART(uint32_t moduleInstance)
{
...
void enviarByte_UART(uint32_t moduleInstance, char c)
{
...
void putsf_UART(uint32_t moduleInstance, char *s)
{
    char c;

    while (c = *s++)    // El apuntador va cambiando su valor.
        enviarByte_UART(moduleInstance, c);
}

/****************************************************************************************************
 * Function: convertirNumToString_UART
 * Preconditions: None
...
void convertirNumToString_UART (float result, char* buffer, int digitos_totales, int max_after_dot_values, int entero)
{
    long int value;
